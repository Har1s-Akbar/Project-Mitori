...
def generate_active_stream():
    prices = ornstein_Uhlenbeck_price(200_000)
    types = np.random.choice([Type.MARKET.value, Type.LIMIT.value], size=200_000, p=[0.5, 0.5])
    sides = np.random.choice([Side.BUY.value, Side.SELL.value], size=200_000)
    quantities = np.random.randint(1, 101, size=200_000)
    user_uuids = np.array([uuid.uuid4() for _ in range(200_000)], dtype=object)
    active_stream = []
